...
namespace kinova
{
        ...
        {
        		// CHITT ADDED THESE //
        		if(current_pose_pub.getNumSubscribers() > 0)
        		{
    	    		std_msgs::Float64MultiArray __p;
    	    		__p.data.resize(6);
        			__p.data = JTAC_jaco->getPose();
        			current_pose_pub.publish(__p);        		
        		}	
        		// CHITT ADDED THESE //
        ...
        void JacoActionController::cartesian_goalCB(CartesianGoalHandle gh)
        {
                ...
                if(gh.getGoal()->poseGoal.header.frame_id != "base_jaco" && gh.getGoal()->poseGoal.header.frame_id != "hand_jaco")
                {
                ...
                // CHITT ADDED THESE TO ENABLE RELATIVE MOVEMENT OF THE JACO ARM.
                if(gh.getGoal()->poseGoal.header.frame_id == "hand_jaco")
                {
                	std::vector<double> __current_pose = JTAC_jaco->getPose();
                	desired_pose.at(0) += __current_pose[0];
                	desired_pose.at(1) += __current_pose[1];
                	desired_pose.at(2) += __current_pose[2];
                	desired_pose.at(3) += __current_pose[3];
                	desired_pose.at(4) += __current_pose[4];
                	desired_pose.at(5) += __current_pose[5];
                }
				// CHITT ADDED THESE TO ENABLE RELATIVE MOVEMENT OF THE JACO ARM.
